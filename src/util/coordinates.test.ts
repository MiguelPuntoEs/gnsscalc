...
import { geo2utm, geo2maidenhead, geo2geohash } from './coordinates';
...
describe('geo2utm', () => {
  it('converts Madrid coordinates correctly', () => {
    ...
    const { zone, hemisphere, easting, northing } = geo2utm(lat, lon);
    expect(zone).toBe(30);
  ...
  it('converts Sydney (southern hemisphere) correctly', () => {
    ...
    const { zone, hemisphere, northing } = geo2utm(lat, lon);
    expect(zone).toBe(56);
...
describe('geo2maidenhead', () => {
  ...
  it('returns 6 characters', () => {
    const mh = geo2maidenhead(deg2rad(0), deg2rad(0));
    expect(mh).toHaveLength(6);
  });
});

describe('geo2geohash', () => {
  it('returns correct length', () => {
    const gh = geo2geohash(deg2rad(48.8566), deg2rad(2.3522));
    expect(gh).toHaveLength(8);
  });

  it('returns correct length with custom precision', () => {
    const gh = geo2geohash(deg2rad(48.8566), deg2rad(2.3522), 5);
  ...
  it('encodes known location (London starts with gcpv)', () => {
    const gh = geo2geohash(deg2rad(51.5074), deg2rad(-0.1278));
    expect(gh.startsWith('gcpv')).toBe(true);
  });

  it('only uses valid base32 characters', () => {
    const gh = geo2geohash(deg2rad(40.4168), deg2rad(-3.7038));
