...
import { systemCmp, systemName } from 'gnss-js/rinex';
...
export const DARK_THEME: ChartTheme = {
  ...
  text: 'rgba(208,208,211,0.5)',
  label: 'rgba(208,208,211,0.35)',
  sysColor: (sys) => SYSTEM_COLORS[sys] ?? '#7c8aff',
  trackOpacity: 0.3,
  trackOpacityDim: 0.1,
  maskColor: 'rgba(255,80,80,0.35)',
  areaFill: 'rgba(124,138,255,0.1)',
  linePrimary: '#7c8aff',
  dimDot: 'rgba(160,160,170,0.3)',
};
  ...
  for (const prn of prns) {
    ...
    for (let i = 0; i < times.length; i++) {
      ...
      if (!pt || pt.el < elevMaskRad) {
        if (path) {
          svg += `<path d="${path}" fill="none" stroke="${color}" stroke-width="1.2" opacity="${opacity}"/>`;
          path = '';
        }
        inSeg = false;
        continue;
...
export function renderMultipathElevSvg(
  ...
  for (const prn of allPositions.prns) {
    ...
    if (ts.length > 0)
      elevMaps.set(prn, { times: ts, els: Float64Array.from(es) });
  }
  ...
  const elevLookup = (prn: string, time: number): number | null => {
    ...
    const idx =
      Math.abs(times[prev]! - time) < Math.abs(times[lo]! - time) ? prev : lo;
    if (Math.abs(times[idx]! - time) > 60000) return null;
    return els[idx]!;
  };

  const BIN = 5,
    numBins = Math.ceil(90 / BIN);
  ...
  const systems = [...sysSet].sort(
    (a, b) => 'GRECIJS'.indexOf(a) - 'GRECIJS'.indexOf(b),
  );
  ...
  for (const sys of systems) {
    ...
    for (let i = 0; i < numBins; i++) {
      const b = sysBins[i]!;
      if (b.count >= 5) {
        const rms = Math.sqrt(b.sumSq / b.count);
        values.push(rms);
  ...
  const xScale = (bin: number) =>
    margin.left + ((bin * BIN + BIN / 2) / 90) * pw;
  ...
  // Legend (top-right)
  svg += renderLegend(
    systems.map((s) => ({ label: systemName(s), color: theme.sysColor(s) })),
    W - margin.right,
    margin.top + 2,
    theme,
  );
...
export function renderCn0TimelineSvg(
  ...
  const yScale = (v: number) =>
    margin.top + ph - ((v - yMin) / (yMax - yMin)) * ph;
  ...
  for (let s = 0; s < nSys; s++) {
    ...
    if (path)
      svg += `<path d="${path}" fill="none" stroke="${color}" stroke-width="1.2" opacity="0.85"/>`;
  }
...
export function renderSatCountSvg(
  ...
  for (let i = 0; i < nEpochs; i++)
    if (totalSats[i]! > maxSats) maxSats = totalSats[i]!;
  ...
  if (areaPath) {
    const lastX = xScale(
      times[Math.min(nEpochs - 1, Math.ceil((nEpochs - 1) / step) * step)]!,
    );
    areaPath += `L${lastX.toFixed(1)},${yScale(0).toFixed(1)}Z`;
    svg += `<path d="${areaPath}" fill="${theme.areaFill}"/>`;
  }
  if (path)
    svg += `<path d="${path}" fill="none" stroke="${theme.linePrimary}" stroke-width="1.2"/>`;
...
export function renderCn0DistributionSvg(
  ...
  for (let i = 0; i <= yTicks; i++) {
    ...
    svg += `<line x1="${margin.left}" y1="${y}" x2="${W - margin.right}" y2="${y}" stroke="${theme.grid}" stroke-width="0.5"/>`;
    if (val > 0)
      svg += `<text x="${margin.left - 5}" y="${y + 3}" text-anchor="end" fill="${theme.label}" font-family="${MONO}" font-size="7">${val >= 1000 ? (val / 1000).toFixed(0) + 'k' : Math.round(val)}</text>`;
  }
...
export function renderCycleSlipRateSvg(
  ...
  stats.sort(
    (a, b) => systemCmp(a.system, b.system) || a.label.localeCompare(b.label),
  );
...
export function renderMultipathPerSatSvg(
  ...
  sats.sort(
    (a, b) => systemCmp(a.prn[0]!, b.prn[0]!) || a.prn.localeCompare(b.prn),
  );
  ...
  for (const s of sats) {
    ...
    sysSeen.add(s.prn[0]!);
    legendItems.push({
      label: systemName(s.prn[0]!),
      color: theme.sysColor(s.prn[0]!),
    });
  }
...
export function renderElevCn0Svg(
  ...
  for (let pi = 0; pi < activeBands.length; pi++) {
    ...
    for (const yVal of [0, 10, 20, 30, 40, 50, 60]) {
      ...
      if (col === 0)
        svg += `<text x="${ox - 4}" y="${y + 3}" text-anchor="end" fill="${theme.label}" font-family="${MONO}" font-size="6">${yVal}</text>`;
    }
    ...
    for (const sys of sysList) {
      ...
      if (path)
        svg += `<path d="${path}" fill="none" stroke="${color}" stroke-width="1.2"/>`;
    }
