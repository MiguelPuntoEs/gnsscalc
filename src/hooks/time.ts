...
import type { TimeResult } from '@/types/time';
import {
  getBdsTime,
  getDayOfWeek,
  getDayOfYear,
...
export default function useTimeCalculator(date: Date): TimeResult {
  const result = useMemo(() => {
    ...
    return {
      ...
      timeOfWeek: getTimeOfWeek(date) / MILLISECONDS_IN_SECOND,
      dayOfYear: getDayOfYear(date),
      weekOfYear: getWeekOfYear(date),
      timeOfDay: getTimeOfDay(date) / MILLISECONDS_IN_SECOND,
      ...
      leapTaiUtc: getLeap(date),
      leapGpsUtc: getGpsLeap(date),
      ...
      timeTai: taiDateObj
        ? (taiDateObj.toISOString().split('T')[1]?.slice(0, -1) ?? '')
        : '',
      dateTT: ttDateObj ? (ttDateObj.toISOString().split('T')[0] ?? '') : '',
      timeTT: ttDateObj
        ? (ttDateObj.toISOString().split('T')[1]?.slice(0, -1) ?? '')
        : '',
      dateUtc: utcDateObj ? (utcDateObj.toISOString().split('T')[0] ?? '') : '',
      timeUtc: utcDateObj
        ? (utcDateObj.toISOString().split('T')[1]?.slice(0, -1) ?? '')
        : '',
