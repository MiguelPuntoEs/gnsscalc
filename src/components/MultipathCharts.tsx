    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-xl bg-bg-raised/60 border border-border/40 p-4">
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-3">
          {(selectedSignal
            ? result.signalStats.filter(
                (s) => `${s.system}-${s.band}-${s.refBand}` === selectedSignal,
              )
            : result.signalStats
          ).map((s) => (
            <div
              key={`${s.system}-${s.band}-${s.refBand}`}
              className="rounded-lg bg-fg/[0.03] px-3 py-2"
            >
              ...
              >
                {s.rms.toFixed(3)}
                <span className="text-xs font-normal text-fg/30 ml-0.5">m</span>
              </div>
              <div className="text-[9px] text-fg/25">
                {s.satellites} SVs &middot; {s.count.toLocaleString()} obs
              </div>
            </div>
          ))}
        </div>
