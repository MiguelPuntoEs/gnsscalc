  ...
  return (
    ...
    >
      <svg
        ...
        viewBox="0 0 16 16"
        fill="currentColor"
        className="size-3 text-fg/20 group-hover:text-fg/40 transition-colors"
      >
        <path
          fillRule="evenodd"
          d="M15 8A7 7 0 1 1 1 8a7 7 0 0 1 14 0Zm-6 3.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0ZM7.293 5.293a1 1 0 1 1 .99 1.667c-.169.108-.308.245-.408.404a.75.75 0 1 0 1.25.834c.2-.3.478-.557.803-.747A2.5 2.5 0 1 0 5.5 5.5a.75.75 0 0 0 1.5 0 1 1 0 0 1 .293-.707Z"
          clipRule="evenodd"
        />
      </svg>
...
export default function FilterBar({
  ...
  // Ref to latest filters so toggle callbacks don't depend on `filters` directly,
  // avoiding re-creation (and child re-renders) on every filter change.
  ...
  const toggleSystem = useCallback(
    (sys: string) => {
      ...
      const ex = f.excludedSystems;
      const next = ex.includes(sys)
        ? ex.filter((s) => s !== sys)
        : [...ex, sys];
      onChange({ ...f, excludedSystems: next });
    },
    [onChange],
  );
  ...
  const togglePrn = useCallback(
    (prn: string) => {
      const f = filtersRef.current;
      const ex = f.excludedPrns;
      const next = ex.includes(prn)
        ? ex.filter((p) => p !== prn)
        : [...ex, prn];
      onChange({ ...f, excludedPrns: next });
    },
    [onChange],
  );
  ...
  const toggleSignalType = useCallback(
    (type: string) => {
      const f = filtersRef.current;
      const ex = f.excludedSignalTypes;
      const next = ex.includes(type)
        ? ex.filter((t) => t !== type)
        : [...ex, type];
      onChange({ ...f, excludedSignalTypes: next });
    },
    [onChange],
  );
  ...
  const toggleBand = useCallback(
    (band: string) => {
      const f = filtersRef.current;
      const ex = f.excludedBands;
      const next = ex.includes(band)
        ? ex.filter((b) => b !== band)
        : [...ex, band];
      onChange({ ...f, excludedBands: next });
    },
    [onChange],
  );

  const toggleSignal = useCallback(
    (sys: string, sig: string) => {
      const f = filtersRef.current;
      const prev = f.excludedSignalsPerSystem;
      ...
      onChange({ ...f, excludedSignalsPerSystem: updated });
    },
    [onChange],
  );
  ...
  const filterCount = useMemo(
    () =>
      filters.excludedSystems.length +
        ...
        <div className="px-3 pb-3 pt-1 border-t border-border/20 flex flex-col gap-3">
          ...
          <div className="flex items-start gap-2 rounded-md bg-accent/5 border border-accent/15 px-2.5 py-1.5 text-[10px] text-fg/50 leading-relaxed">
            <svg
              ...
              viewBox="0 0 16 16"
              fill="currentColor"
              className="size-3.5 shrink-0 mt-0.5 text-accent/50"
            >
              <path
                fillRule="evenodd"
            ...
            Filters apply to both the analysis (charts, QA, statistics) and
            exported files. The original data is never modified.
          </div>

          {/* Signal types */}
          <div>
            <div className="text-[10px] text-fg/30 mb-1.5 uppercase tracking-wider flex items-center">
              Signal types
              <HintIcon hint="Filter by measurement type. Code = pseudorange, Phase = carrier phase, Doppler = frequency shift, SNR = signal strength. Applies across all constellations." />
            </div>
          ...
          <div>
            <div className="text-[10px] text-fg/30 mb-1.5 uppercase tracking-wider flex items-center">
              Frequency bands
              <HintIcon hint="Exclude entire frequency bands (L1, L2, L5, etc.) across all constellations. Band numbering follows the RINEX convention and varies per system." />
            </div>
            <div className="flex flex-wrap gap-1.5">
          ...
          <div>
            <div className="text-[10px] text-fg/30 mb-1.5 uppercase tracking-wider flex items-center">
              ...
              <HintIcon hint="Toggle individual signals per constellation. Each button represents a RINEX signal (band + tracking attribute, e.g. 1C = L1 C/A). Excludes all observation types (C/L/D/S) for that signal. Hover a signal for its name." />
            </div>
                ...
                return (
                  <div key={sys} className={sysExcluded ? 'opacity-30' : ''}>
                    ...
                    >
                      {systemName(sys)}
                      {sysExcluded && (
                        <span className="text-fg/30 ml-1">(excluded)</span>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-1">
                      {sigs.map((sig) => {
                        ...
                        const globallyExcluded =
                          filters.excludedBands.includes(band);
                        const perSysExcluded = sysExSigs.includes(sig);
                        const excluded =
                          sysExcluded || globallyExcluded || perSysExcluded;
          ...
          <div>
            <div className="text-[10px] text-fg/30 mb-1.5 uppercase tracking-wider flex items-center">
              Satellites
              <HintIcon hint="Exclude individual satellites (PRNs). Useful for removing specific vehicles with known issues, e.g. unhealthy or maneuvering satellites." />
            </div>
            <div className="flex flex-col gap-2">
              {allSystems.map((sys) => {
                ...
                return (
                  <div key={sys} className={sysExcluded ? 'opacity-30' : ''}>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {prns.map((prn) => {
                        const excluded =
                          sysExcluded || filters.excludedPrns.includes(prn);
                        return (
                          <button
                            key={prn}
                            type="button"
          ...
          <div className="grid grid-cols-[1fr_1fr_1fr] gap-3 items-start">
            <div>
              <div className="text-[10px] text-fg/30 mb-1 uppercase tracking-wider flex items-center">
                Time range
                <HintIcon hint="Crop the dataset to a time window. Times are in UTC. Useful for isolating a specific session or event." />
              </div>
              <div className="flex flex-col gap-1">
                <input
                  ...
                  value={
                    filters.timeStart != null
                      ? toLocalISO(filters.timeStart)
                      : timeStart != null
                        ? toLocalISO(timeStart)
                        : ''
                  }
                  onChange={(e) => {
                    const v = e.target.value
                      ? new Date(e.target.value + 'Z').getTime()
                      : null;
            ...
            <div>
              <div className="text-[10px] text-fg/30 mb-1 uppercase tracking-wider flex items-center">
                Sampling
                <HintIcon hint="Decimate epochs to a lower rate. Keeps only epochs that match the selected interval. Reduces data volume for export or faster analysis." />
              </div>
              <select
                className="w-full bg-bg/60 border border-border/30 rounded px-2 py-1 text-[11px] text-fg/70 focus:border-accent/50 outline-none"
              ...
              >
                <option value="">
                  Original{interval != null ? ` (${interval}s)` : ''}
                </option>
            ...
            <div>
              <div className="text-[10px] text-fg/30 mb-1 uppercase tracking-wider flex items-center">
                Sparse removal
                <HintIcon hint="Remove signals that appear in fewer than X% of epochs. Cleans up sporadically tracked signals that add noise without useful data." />
              </div>
              <select
                className="w-full bg-bg/60 border border-border/30 rounded px-2 py-1 text-[11px] text-fg/70 focus:border-accent/50 outline-none"
                ...
                onChange={(e) =>
                  onChange({
                    ...filters,
                    sparseThreshold: Number(e.target.value),
                  })
                }
              >
