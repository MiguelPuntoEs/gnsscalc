    ...
    <ChartCard title="Data completeness per signal (%)" height={Math.max(160, data.length * 24 + 40)}>
      <ResponsiveContainer>
        <BarChart data={data} layout="vertical" margin={{ top: 5, right: 20, left: 10, bottom: 0 }}>
          ...
          />
          <Bar dataKey="percent" name="Completeness" radius={[0, 4, 4, 0]} barSize={16}>
            {data.map((d, i) => {
              const alpha = d.percent > 95 ? 0.8 : d.percent > 80 ? 0.6 : 0.4;
              return <Cell key={i} fill={systemColor(d.system)} fillOpacity={alpha} />;
            })}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
...
function CompletenessHeatmap({ result, selectedSys }: { result: CompletenessResult; selectedSys: string | null }) {
  ...
  const { prns, codes, grid } = useMemo(() => {
    const filtered = result.cells.filter(c => !selectedSys || c.system === selectedSys);
    const prnSet = new Set<string>();
    const codeSet = new Set<string>();
    for (const c of filtered) { prnSet.add(c.prn); codeSet.add(c.code); }
  ...
  const draw = useCallback((canvas: HTMLCanvasElement) => {
    ...
    for (let r = 0; r < numRows; r++) {
      for (let c = 0; c < numCols; c++) {
        const pct = grid[r]![c];
        ctx.fillStyle = pct !== undefined ? completeColor(pct) : 'rgba(255,255,255,0.03)';
        ctx.fillRect(LABEL_W + c * cellW, HEADER_H + r * cellH, cellW - 0.5, cellH - 0.5);
      }
    }
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    tip.textContent = pct !== undefined
      ? `${prns[row]} ${codes[col]}: ${pct.toFixed(1)}%`
      : `${prns[row]} ${codes[col]}: —`;
  }, [prns, codes, grid]);

  if (prns.length === 0) return null;
  const chartH = Math.min(Math.max(prns.length * 14 + HEADER_H + LEGEND_H + 10, 160), 800);
...
export default function CompletenessCharts({ result }: { result: CompletenessResult }) {
            ...
            .map(sys => {
              ...
              return (
                <div key={sys} className="rounded-lg bg-white/[0.03] px-3 py-2">
                  ...
                  <div className="text-[9px] text-fg/25">{stats.reduce((a, s) => a + s.satellites, 0)} SVs</div>
                </div>
              );
            })}
        </div>
