...
function SlipRateBar({ stats }: { stats: CycleSlipResult['signalStats'] }) {
  ...
  return (
    <ChartCard
      title="Cycle slip rate per signal (slips / 1000 epochs)"
      height={Math.max(160, data.length * 36 + 40)}
    >
      <ResponsiveContainer>
        <BarChart
          data={data}
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(_value, _name, item) => {
              ...
              return [
                `${d.slipRate.toFixed(2)} / 1000 ep  (${d.totalSlips} slips, ${d.satellites} SVs)`,
                'Rate',
              ];
            }}
          />
...
function SatSlipBar({
  ...
  return (
    <ChartCard
      title="Cycle slips per satellite"
      height={Math.max(160, data.length * 14 + 40)}
    >
      <ResponsiveContainer>
        <BarChart
          data={data}
        ...
        >
          ...
          <Tooltip
            {...theme.tooltipStyle}
            formatter={(value) => [`${String(value)} slips`, 'Count']}
          />
...
function SlipTimeline({
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (row < 0 || row >= allPrns.length || x < LABEL_W) {
      ...
      if (!closest || minDist > duration * 0.02) {
        tip.style.display = 'none';
        return;
      }
...
export default function CycleSlipCharts({
  result,
}: {
  result: CycleSlipResult;
}) {
      ...
      <div className="rounded-xl bg-bg-raised/60 border border-border/40 p-4">
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] gap-3">
          <div className="rounded-lg bg-fg/[0.03] px-3 py-2">
            ...
            <div className="text-lg font-semibold text-fg/80">
          ...
          {(selected
            ...
            <div key={s.label} className="rounded-lg bg-fg/[0.03] px-3 py-2">
              ...
              <div
                ...
                style={{ color: systemColor(s.system) }}
              >
                {s.totalSlips}
                <span className="text-xs font-normal text-fg/30 ml-1">
                  {s.slipRate.toFixed(1)}/k
                </span>
              </div>
              <div className="text-[9px] text-fg/25">
                {s.satellites} SVs &middot; {s.totalEpochs.toLocaleString()} ep
              </div>
            </div>
          ))}
        </div>
