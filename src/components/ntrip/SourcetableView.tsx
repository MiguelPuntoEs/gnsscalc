...
export default function SourcetableView({
  ...
  const [sortKey, setSortKey] = useState<
    'mountpoint' | 'format' | 'navSystem' | 'country' | 'bitrate'
  >('mountpoint');
    ...
    <div className="space-y-4">
      ...
      <div className="card">
        <div className="card-fields">
          <span>Caster</span>
          <span className="text-sm text-fg/80 font-mono">
            {host}:{port}
          </span>
          <span>Protocol</span>
          ...
          {networks.length > 0 && (
            <>
              ...
              <span className="text-sm text-fg/80">
                {networks.map((n) => n.identifier).join(', ')}
              </span>
            </>
          )}
          <span>Countries</span>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-bg-raised text-fg/50 text-left">
              <th
                className="px-3 py-2 cursor-pointer hover:text-fg/80 whitespace-nowrap"
              ...
              <th
                className="px-3 py-2 cursor-pointer hover:text-fg/80 whitespace-nowrap text-right"
          ...
          <tbody>
            {paginatedRows.map((s) => (
              <tr
                key={s.mountpoint}
                className="border-t border-border/20 hover:bg-bg-raised/50 transition-colors cursor-pointer"
                onClick={() => {
                  if (!streamConnecting) onStreamConnect(s.mountpoint);
                }}
              >
                <td className="px-3 py-1.5 font-mono font-medium whitespace-nowrap">
                  ...
                  {s.identifier && (
                    <span className="font-normal text-fg/30 ml-1.5">
                      {s.identifier}
                    </span>
                  )}
                </td>
                <td className="px-3 py-1.5 text-fg/60 whitespace-nowrap">
                  {s.format}
                </td>
                <td className="px-3 py-1.5 whitespace-nowrap">
                  {s.navSystem.split('+').map((sys) => (
                    <span
                      key={sys}
                      className="inline-block mr-1 text-[10px] font-semibold"
                      style={{
                        color: CONSTELLATION_COLORS[sys.trim()] ?? '#94a3b8',
                      }}
                    >
                      {sys.trim()}
                    </span>
                ...
                <td className="px-3 py-1.5 text-fg/40">
                  {CARRIER_LABELS[s.carrier] ?? s.carrier}
                </td>
                <td className="px-3 py-1.5 text-fg/50 text-right font-mono tabular-nums">
                  {s.bitrate > 0 ? formatBitrate(s.bitrate) : '—'}
                </td>
                <td className="px-3 py-1.5 text-fg/40">
                  {AUTH_LABELS[s.authentication] ?? s.authentication}
                </td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr>
                <td colSpan={7} className="px-3 py-6 text-center text-fg/30">
      ...
      <p className="text-[10px] text-fg/25 text-center">
        {filtered.length} of {streams.length} streams — click a mountpoint to
        connect
      </p>
    </div>
  );
