...
import type { StationMeta } from 'gnss-js/rtcm3';
...
export default function StationInfoCard({
  ...
  const geo = useMemo(() => {
    ...
    if (
      ...
      return {
        lat: streamEntry.latitude,
        lon: streamEntry.longitude,
  ...
  if (meta.stationId !== null)
    rows.push(['Station ID', String(meta.stationId)]);
  if (streamEntry?.identifier)
    rows.push(['Identifier', streamEntry.identifier]);
  ...
  if (geo) {
    ...
    rows.push([
      'Position',
      `${latStr}, ${lonStr}${geo.alt !== null ? `, ${geo.alt.toFixed(1)} m` : ''}`,
    ]);
    if (meta.position)
      rows.push([
        'ECEF',
        `${meta.position[0].toFixed(4)}, ${meta.position[1].toFixed(4)}, ${meta.position[2].toFixed(4)} m`,
      ]);
  }
  if (meta.itrf !== null && meta.itrf > 0)
    rows.push(['ITRF', `${meta.itrf < 100 ? '20' : ''}${meta.itrf}`]);
  if (meta.antennaHeight !== null)
    rows.push(['Antenna Height', `${meta.antennaHeight.toFixed(4)} m`]);
  if (meta.antennaType)
    rows.push([
      'Antenna',
      meta.antennaType + (meta.antennaSerial ? ` (${meta.antennaSerial})` : ''),
    ]);
  if (meta.receiverType)
    rows.push([
      'Receiver',
      meta.receiverType +
        (meta.receiverFirmware ? ` / ${meta.receiverFirmware}` : ''),
    ]);
      ...
      <div className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1">
        {rows.map(([label, value]) => (
          <div key={label} className="contents">
            ...
            <span className="text-[11px] text-fg/80 font-mono truncate">
              {value}
            </span>
          </div>
        ))}
