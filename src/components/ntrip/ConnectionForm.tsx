...
export function loadSavedConnection(): { host: string; port: string; version: NtripVersion; username: string; password: string } {
  try {
  ...
  return { host: '', port: '2101', version: '2.0', username: '', password: '' };
}
...
export default function ConnectionForm({ onConnect, loading }: ConnectionFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      localStorage.setItem(NTRIP_STORAGE_KEY, JSON.stringify({ host: h, port, version, username, password }));
    } catch { /* ignore */ }
