...
import type { NtripVersion } from 'gnss-js/ntrip';
    ...
    if (raw) {
      ...
      if (data && typeof data === 'object') {
        ...
        return {
          ...
          port: typeof d.port === 'string' ? d.port : '2101',
          version: d.version === '1.0' ? ('1.0' as const) : ('2.0' as const),
          username: typeof d.username === 'string' ? d.username : '',
          password:
            d.remember && typeof d.password === 'string' ? d.password : '',
          remember: typeof d.remember === 'boolean' ? d.remember : false,
        };
      }
    }
  } catch {
    ...
    <form ref={formRef} onSubmit={handleSubmit} className="card">
      <div className="card-fields">
        ...
        <div className="flex gap-2 items-center">
          <label className="flex items-center gap-1 !text-xs !font-normal !text-fg/70 !normal-case !tracking-normal cursor-pointer">
            <input
              ...
              name="version"
              value="2.0"
              defaultChecked={saved.version === '2.0'}
              className="!w-auto !h-auto !min-w-0 accent-accent"
            />
            NTRIP 2.0
          </label>
          <label className="flex items-center gap-1 !text-xs !font-normal !text-fg/70 !normal-case !tracking-normal cursor-pointer">
            <input
              ...
              name="version"
              value="1.0"
              defaultChecked={saved.version === '1.0'}
              className="!w-auto !h-auto !min-w-0 accent-accent"
            />
            NTRIP 1.0
          </label>
        ...
        <p className="col-span-full text-[10px] text-fg/30 -mt-1 mb-1 leading-snug">
          Credentials are sent directly from your browser to the caster. Nothing
          is transmitted to or stored on our servers.
        </p>

        <label htmlFor="ntrip-username">Username</label>
        <input
          id="ntrip-username"
          type="text"
          name="username"
