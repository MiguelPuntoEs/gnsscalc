    ...
    <div className="rounded-lg border border-border/40 bg-bg-raised/30 p-3">
      <div className="flex items-center gap-3 mb-2">
        ...
        <span className="text-[10px] text-fg/30 font-mono">
          {geo.lat.toFixed(4)}° {geo.lat >= 0 ? 'N' : 'S'}, {geo.lon.toFixed(4)}° {geo.lon >= 0 ? 'E' : 'W'}, {geo.alt.toFixed(1)}m
        </span>
        <span className="text-[10px] text-fg/40 ml-auto">{aboveMask} sats &gt;5°</span>
      </div>
      <div className="relative max-w-[400px] mx-auto">
        <canvas
          ...
          onMouseMove={(e) => {
            ...
            for (const sp of satPositions.current) {
              ...
              if (d2 < closestDist) { closest = sp; closestDist = d2; }
            }
            if (closest) {
              setHovered({ sat: closest.sat, x: e.clientX - rect.left, y: e.clientY - rect.top });
            } else {
              setHovered(null);
            }
          }}
        ...
        {hovered && (
          <div
            ...
            style={{
              left: Math.min(hovered.x + 12, (canvasRef.current?.getBoundingClientRect().width ?? 400) - 120),
              top: hovered.y < 60 ? hovered.y + 12 : hovered.y - 60,
            }}
          >
            <div className="font-semibold text-xs mb-0.5" style={{ color: SYSTEM_COLORS[hovered.sat.prn.charAt(0)] ?? '#7c8aff' }}>
              {hovered.sat.prn}
            </div>
            <div className="text-fg/60">
              Az {(hovered.sat.az * 180 / Math.PI).toFixed(1)}° &nbsp; El {(hovered.sat.el * 180 / Math.PI).toFixed(1)}°
            </div>
            {hovered.sat.cn0 !== undefined && hovered.sat.cn0 > 0 && (
              <div className="text-fg/60">C/N₀ {hovered.sat.cn0.toFixed(1)} dB-Hz</div>
            )}
          </div>
        )}
      </div>
