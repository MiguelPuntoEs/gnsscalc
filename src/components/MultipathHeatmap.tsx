...
export default function MultipathHeatmap({
  result,
  selectedSignal,
}: {
  ...
  const filtered = useMemo(() => {
    ...
    return result.series.filter(
      (s) => s.system === sys && s.band === band && s.refBand === refBand,
    );
    ...
    (canvas: HTMLCanvasElement) => {
      ...
      for (let r = 0; r < numRows; r++) {
        ...
        for (const p of s.points) {
          const col = Math.min(
            numCols - 1,
            Math.floor(((p.time - tMin) / duration) * numCols),
          );
          bins[col]! += p.mp;
          counts[col]!++;
        }
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (row < 0 || row >= prnRows.length || x < LABEL_W) {
