...
import type { NtripStream } from '../util/ntrip';
...
function FitBounds({ streams }: { streams: NtripStream[] }) {
  ...
  useMemo(() => {
    ...
    map.fitBounds([[minLat, minLng], [maxLat, maxLng]], { padding: [30, 30], maxZoom: 12 });
  }, [streams, map]);
  return null;
}
...
function getColor(format: string): string {
  const key = Object.keys(FORMAT_COLORS).find(k => format.toUpperCase().startsWith(k.toUpperCase()));
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-border/40">
      ...
      >
        ...
        {streams.map(s => (
          <CircleMarker
            ...
            radius={5}
            pathOptions={{ color: getColor(s.format), fillColor: getColor(s.format), fillOpacity: 0.7, weight: 1 }}
          >
            <Popup>
              <div className="text-xs leading-relaxed text-gray-900">
                <strong className="font-semibold">{s.mountpoint}</strong>
                {s.identifier && <div className="text-gray-500">{s.identifier}</div>}
                <div>{s.format} {s.formatDetails && `(${s.formatDetails})`}</div>
                <div>{s.navSystem}</div>
                <div>{s.latitude.toFixed(4)}, {s.longitude.toFixed(4)}</div>
      ...
      <div className="bg-bg-raised/60 px-3 py-1.5 flex flex-wrap gap-x-4 gap-y-1 text-[10px] text-fg/50">
        {[['RTCM 3.x', '#4ade80'], ['RTCM 2.x', '#fbbf24'], ['RAW', '#60a5fa'], ['Other', '#94a3b8']].map(([label, color]) => (
