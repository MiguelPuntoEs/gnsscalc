...
export type TrackPoint = { az: number; el: number; lat: number; lon: number; epoch: number };
...
export default function PolarSkyPlot({
  ...
  const draw = useCallback((canvas: HTMLCanvasElement) => {
    ...
    const drawSat = (prn: string, sat: SatAzEl, isObserved: boolean) => {
      const color = isObserved ? (SYSTEM_COLORS[prn.charAt(0)] ?? '#7c8aff') : t.unobservedDot;
      ...
      ctx.fill();
      ctx.shadowBlur = 0;
      ctx.fillStyle = isObserved ? t.canvasText + '0.8)' : t.canvasText + '0.2)';
