...
import type { Ephemeris } from '../util/nav';
...
import { SYSTEM_COLORS, formatUTCTime } from '../util/gnss-constants';
...
function ConstellationGrid({ ag }: { ag: AvailabilityGrid }) {
  ...
  const stateLabel = (state: number): string => {
    switch (state) {
      case CellState.EphHealthy: return 'Healthy';
      case CellState.EphUnhealthy: return 'Unhealthy';
      case CellState.ObsEphHealthy: return 'Observed + healthy';
      case CellState.ObsEphUnhealthy: return 'Observed + unhealthy';
      case CellState.ObsNoEph: return 'Observed, no ephemeris';
      case CellState.NotObsHasEph: return 'Has eph, not observed';
