...
function greatCirclePoints(
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    points.push([rad2deg(Math.atan2(z, Math.sqrt(x * x + y * y))), rad2deg(Math.atan2(y, x))]);
  }
  return points;
}
...
export default function PositionMap({
  ...
  const arcPoints = useMemo(
    () => greatCirclePoints(ref[0], ref[1], pos[0], pos[1]),
    [pos[0], pos[1], ref[0], ref[1]]
  );

  const rhumbPoints = useMemo(
    () => rhumbLinePoints(ref[0], ref[1], pos[0], pos[1]),
    [pos[0], pos[1], ref[0], ref[1]]
  );

  return (
    <div className="relative rounded-xl border border-border/60 overflow-hidden" role="region" aria-label="Position map" style={{ height: 350 }}>
      <MapContainer
        center={pos}
        zoom={10}
      ...
      >
        ...
        <Polyline
          ...
          pathOptions={{ color: '#ff7c7c', weight: 2, opacity: 0.6, dashArray: '6 4' }}
        />
      ...
      <div className="absolute bottom-2 left-2 z-[1000] flex flex-col gap-1 rounded-lg bg-bg-raised/90 backdrop-blur-sm border border-border/60 px-2.5 py-1.5 text-[10px] font-medium text-fg/70">
        ...
        <div className="flex items-center gap-1.5">
          <span className="inline-block w-4 h-0.5 rounded-full border-t border-dashed" style={{ borderColor: '#ff7c7c' }} />
          Loxodromic
        </div>
      </div>
    </div>
