...
function SatelliteHeatmap({
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      tip.textContent =
        val != null && !isNaN(val)
          ? `${prn} @ ${timeStr}: ${val.toFixed(1)} dB-Hz`
          : `${prn} @ ${timeStr}: —`;
    },
...
export default function RinexCharts({
  ...
  const formatTime = useCallback(
    (time: number) => formatUTCTime(new Date(time)),
    [],
  );

  const data = useMemo(
    () => gridToChartRows(grid, formatTime),
    [grid, formatTime],
  );
  ...
  const chartPlaceholder = (
    <div className="rounded-xl border border-border/40 bg-bg-raised/60 p-4">
      ...
      <div className="flex items-center justify-center" style={{ height: 180 }}>
        <svg
          className="size-5 animate-spin text-fg/20"
          viewBox="0 0 24 24"
          fill="none"
        >
          <circle
